...
class RouteDecision(BaseModel):
    ...
    destination: Literal[
        ...
        description=(
            ...
            "Choose 'career_retrieve_node' for jobs/vacancies,hiring,careers, job openings. "
            "Choose direct_node for greetings, small talk, or general world-knowledge questions that are not specific to LMKR. "
            "Choose static_retrieve_node ONLY if the question is about LMKR, its products, platforms, services, or company information, TRVERSE, GVERSE, GVERSE geographix, Xposim, contact informations etc.\n"
        )
    )
    ...
    for line in lines:
        if (
            ...
            and not any(
                bad in line.lower()
                for bad in [
                    "apply", "location", "department",
                    "bamboohr", "source", "scraped_at"
                ]
            )
        ):
            jobs.append(line)
...
def career_retrieve_node(state: AgentState):
    ...
    is_list_request = any(p in q for p in [
    ...
    if is_file_fresh(config.careers_output_file, SCRAPE_FRESH_HOURS):
        ...
        print("📄 Loaded careers from cache")
    else:
    ...
    # --------------------------------------------------
    # 🔴 LIST MODE → BYPASS FAISS COMPLETELY
    # --------------------------------------------------
    if is_list_request:
        print("📋 Listing ALL jobs (no FAISS filtering)")
...
def direct_node(state: AgentState):
    print("\nNode: Direct (Small Talk + Introductions)...")

    question = state["question"]

    prompt = f"""
You are an LMKR website assistant.

User Input:
{question}

Rules:
1. You may respond ONLY if the input is:
   - a greeting (hi, hello, good morning, etc.)
   - a farewell (bye, goodbye)
   - a polite phrase (thanks, thank you)
   - a self-introduction (e.g. "my name is X", "I am X")
2. If the user introduces themselves, acknowledge politely and use their name in the reply.
3. If the input asks for an explanation, definition, or description of anything,
   you MUST NOT answer it.
4. If the input is not allowed, respond EXACTLY with:
   "Sorry, I can only help with questions about LMKR, its products, and its services."
5. Be brief, friendly, and conversational.
...
"""
    ...
    if not response:
        response = GeneratedAnswer(
            answer="Hello! How can I help you with LMKR today?",
            sources_used=["Direct"]
        )
    else:
...
8. Answer every job in next line.
...
"""
    ...
    # --------------------------------------------------
# 🔴 FIX FORMATTING FOR JOB LISTS
# --------------------------------------------------
    if list_jobs_mode and response and response.answer:
        response.answer = "\n".join(
        line.strip()
    ...
    if list_jobs_mode:
        response.answer += (
        "\n\nFor more and updated job openings, please visit:\n"
        f"{config.careers_url}"
    )
